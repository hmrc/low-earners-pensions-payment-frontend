...
class UserAllowListConnectorISpec
  extends IntegrationSpecBase with WireMockMethods {
  ...
  ".check" when {
    ...
    val feature = "nino"
    val url = s"/user-allow-list/low-earners-pensions-payment-frontend/$feature/check"
    ...
    "must return true when the server responds OK" in new Test {
      ...
      connector.check(feature, request.value).futureValue shouldBe true
    }
    ...
    "must return false when the server responds NOT_FOUND" in new Test {
      ...
      connector.check(feature, request.value).futureValue shouldBe false
    }
