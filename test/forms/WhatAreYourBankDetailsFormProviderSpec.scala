...
class WhatAreYourBankDetailsFormProviderSpec extends FormSpecBase {
  "WhatAreYourBankDetailsFormProvider" - {
    ...
    "bind" - {
      ...
      ).foreach(handleForMandatoryField(form, "bankAccountDetails"))
      
      handleForOptionalField(form, "bankAccountDetails")(
        "buildingSocietyRollNumber", 1, 18, Seq("!!!"), Seq("ABCDEF"), "^[A-Z0-9]{1,18}$"
      )
      ...
      "should strip dashes from sort code field" in {
        form.bind(Map(
          ...
          "accountNumber" -> "12345678",
          "sortCode" -> "11-22-33",
          "buildingSocietyRollNumber" -> "ABCDEF"
